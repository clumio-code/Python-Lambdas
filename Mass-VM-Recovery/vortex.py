...
def get_VM_backups(base_url, version, token, vmid, vcid, latest=False):
    resource = '/api/backups/vmware/vms'
    filter = {
        "vcenter_id": {"$eq": vcid},
        "vm_id": {"$eq": vmid}
    }
...
def get_PIT_params(base_url, version, token, pit_id):
    url = f'{base_url}/api/backups/vmware/vms/{pit_id}'
    time.sleep(3)
    response = http_get(url, version, token=token)
    return response
...
def recoverVM(base_url, version, token, body):
    url = f'{base_url}/api/restores/vmware/vms'
...
def list_vm(base_url, version, token, vcid,
    ...
    resource = f'/api/datasources/vmware/vcenters/{vcid}/vms'
    filter = {
...
def list_vc(base_url, version, token):
    url = f'{base_url}/api/datasources/vmware/vcenters?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']


def list_tags(base_url, version, token, vcid):
    url = f'{base_url}/api/datasources/vmware/vcenters/{vcid}/tags?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']


def list_vc_pools(base_url, version, token, vcid):
    url = f'{base_url}/api/datasources/vmware/vcenters/{vcid}/resource-pools?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']


def list_vc_datacenters(base_url, version, token, vcid):
    url = f'{base_url}/api/datasources/vmware/vcenters/{vcid}/datacenters?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']


def list_vc_datastores(base_url, version, token, vcid):
    url = f'{base_url}/api/datasources/vmware/vcenters/{vcid}/datastores?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']


def list_vc_folders(base_url, version, token, vcid):
    url = f'{base_url}/api/datasources/vmware/vcenters/{vcid}/folders?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']


def list_vc_networks(base_url, version, token, vcid):
    url = f'{base_url}/api/datasources/vmware/vcenters/{vcid}/networks?limit=500'
    response = http_get(url, version, token=token)
    return response['_embedded']['items']
